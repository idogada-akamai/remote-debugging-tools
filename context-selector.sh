...
get_deployment_label_selector() {
    ...
    json_path=".spec.selector.matchLabels"
    # command="kubectl --context=$context --namespace=$namespace get deploy -o json | jq -r "$json_path""
    # echo $command
...
selector=$(get_deployment_label_selector $context $namespace $deployment)
